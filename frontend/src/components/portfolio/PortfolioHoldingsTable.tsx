...
function resolveDecisionMeta(decision: AIAnalysisItem['decision']): DecisionMeta {
  if (decision === 'BUY') {
    return {
      label: '매수',
      className: 'border-border-strong bg-surface-high text-brand-bright',
    }
  }
  if (decision === 'SELL') {
    return {
      label: '매도',
      className: 'border-border-strong bg-surface-high text-market-negative',
    }
  }
  return {
    label: '관망',
    className: 'border-border-strong bg-surface-high text-warning',
  }
}
          ...
          <table className="min-w-[720px] w-full border-separate border-spacing-0 text-left">
            <thead>
              <tr className="text-xs font-semibold text-content-muted">
                <th className="border-b border-border-subtle px-4 py-3">종목</th>
                <th className="border-b border-border-subtle px-4 py-3">평가금액</th>
                <th className="border-b border-border-subtle px-4 py-3">수익률</th>
                <th className="border-b border-border-subtle px-4 py-3">보유수량</th>
                <th className="border-b border-border-subtle px-4 py-3">AI 판단</th>
              </tr>
            </thead>
            <tbody>
              {coinItems.map((item) => {
                ...
                return (
                  <tr
                    key={currency}
                    className="text-sm text-content transition-colors hover:bg-surface-high"
                  >
                    <td className="border-b border-border-subtle px-4 py-4 font-semibold">
                      {currency}
                    </td>
                    <td className="border-b border-border-subtle px-4 py-4 font-medium">
                      {formatKrw(item.total_value)}
                    </td>
                    <td
                      className={`border-b border-border-subtle px-4 py-4 font-semibold ${resolvePnlClassName(
                        item.pnl_percentage,
                      )}`}
                    >
                      {formatSignedPercent(item.pnl_percentage)}
                    </td>
                    <td className="border-b border-border-subtle px-4 py-4 text-content-secondary">
