...
function PortfolioAiBriefing({
  ...
  const providerLabel =
    briefingEntry?.provider && briefingEntry.provider !== 'local'
      ? `${briefingEntry.provider.toUpperCase()} · ${briefingEntry.model}`
      : null
      ...
      <div>
        ...
        {!isPortfolioLoading ? (
          <div className="space-y-4">
            ...
            {providerLabel ? (
              <div className="inline-flex rounded-md border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
                {providerLabel}
              </div>
            ) : null}
