...
function AiActivityLiveFlow() {
  ...
  const repeatedItems = [...flowItems, ...flowItems]

  return (
    <section
      className="relative h-10 overflow-hidden rounded-lg border border-[#29363a]/80 bg-[#0a0e14]"
      aria-label="AI 활동 라이브 플로우"
    >
      ...
      <div className="quantum-ticker-scroll flex h-full min-w-max items-center gap-10 pl-40 pr-8 font-mono text-xs">
        {repeatedItems.map((item, index) => (
          <div key={`${item.label}-${index}`} className="flex min-w-max items-center gap-2">
