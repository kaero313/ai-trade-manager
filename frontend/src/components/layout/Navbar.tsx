  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-gray-200 bg-white/95 text-gray-900 shadow-sm backdrop-blur dark:border-gray-700 dark:bg-gray-800/95 dark:text-gray-100">
      <div className="mx-auto flex h-16 w-full max-w-full items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex min-w-0 items-center gap-2 text-sm font-medium sm:gap-3 lg:gap-4">
          <Link
            to="/"
            className="rounded-md px-3 py-2 text-gray-600 transition-colors hover:bg-gray-200 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white"
          >
            Dashboard
          </Link>
          <Link
          ...
          <div className="hidden items-center gap-4 lg:flex">
            <span className="whitespace-nowrap text-xs font-semibold text-gray-600 dark:text-gray-300">
              총 자산: <span className="text-gray-900 dark:text-gray-100">{isPortfolioLoading ? '...' : formatKrw(totalNetWorth)}</span>
            </span>
