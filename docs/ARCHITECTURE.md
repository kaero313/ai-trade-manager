...
2. AI가 BUY, SELL, HOLD 판단과 근거를 분석 로그로 커밋하고, 해당 호출에서 저장된 ID를 cycle identity로 확정합니다. 저장 실패 시 해당 종목 cycle을 중단합니다.
3. 실행 단계는 전달받은 ID로 분석 로그를 단건 조회하고 종목 일치를 확인한 뒤 확신도, 진입 게이트, 실거래 매수 잠금, 관측 모드를 다시 확인합니다. 종목별 최신 분석으로 암묵적으로 교체하지 않습니다.
...
- 자동 scheduler와 수동 AI Cycle은 저장된 분석 ID를 명시적으로 전달하며, ID 누락·미존재·종목 불일치는 주문 전에 fail-closed로 차단합니다.
