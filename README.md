...
기존 단일 환경(In-memory) 트레이딩 봇에서 벗어나, 완벽히 분리된 **마이크로서비스 아키텍처**(백엔드 API, 워커 엔진, 슬랙 봇, 프론트엔드 대시보드)로 구축되었습니다. 최근에는 백테스트 엔진과 기술적 지표 시각화(MA, BB, RSI)를 넘어 **UI 폼을 통한 실시간 매매 파라미터 제어(Phase 17)** 기능까지 성공적으로 이식되었습니다.
